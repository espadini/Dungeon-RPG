...
void Monster::displayStats() const {
    ...
    cout << getName() << " [HP: " << getCurrentHP() << "/" << getMaxHP() << "]" << endl;
}
...
void Monster::addLoot(Item* item) {
    ...
    if(!item) {return;} 
    else {loot_table.push_back(item);}
}
...
Skeleton::Skeleton()
    ...
    // TODO: Add loot items
    addLoot(new Weapon("Old Sword", "A relic of times past.", 2));
...
Dragon::Dragon()
    ...
    addLoot(new Weapon("Dragon Slayer Sword", "A dragon-bone crusted hilt laden with a singular wicked dragon tooth.", 10));
    addLoot(new Armor("Dragon Scale Armor", "The thousands of shining scales emit an ominous aura, intimidating those you seek to harm.", 8));
